...
class Add extends React.Component {
  ...
  closeModal() {
    this.setState({modalIsOpen: false, description: '', amount: '', year: 2016, messageFromServer: ''});
  }
  componentDidMount() {
  ...
  insertNewExpense(e) {
    axios.post('/insert', queryString.stringify({desc: e.state.description, amount: e.state.amount, month: e.state.month, year: e.state.year}), {
      headers: {
        "Content-Type": "application/x-www-form-urlencoded"
      }
    }).then(function(res) {
      e.setState({messageFromServer: res.data});
    });
  }
  handleTextChange(e) {
  ...
  render() {
    if (this.state.messageFromServer == '') {
      return (<div>
        <Button bsStyle="success" bsSize="large" onClick={this.openModal}>
